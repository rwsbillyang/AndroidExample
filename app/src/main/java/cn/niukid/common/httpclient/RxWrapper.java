...
public class RxWrapper {
    ...
    private HttpResultHandler subscriber;
    ...
    {
        subscriber = new HttpResultHandler(resultHandler);
     ...
     * 返回结果格式与ResultModel类似，即数据类型为T，是被ResultModel包裹
     * */
    public RxWrapper setObservableUsingResultMapper(Observable observable)
    {
     ...
     * HttpResultHandler的onError将在这时被调用，并对其进行进一步处理，并最终即HttpResultObserver.handleError进行Toast显示处理。
     *
    ...
    public static class ThrowableToObserverSource<T> implements Function<Throwable, Observable<T>>
    {
        ...
        {
            Logger.w("in ThrowableToObserverSource");
